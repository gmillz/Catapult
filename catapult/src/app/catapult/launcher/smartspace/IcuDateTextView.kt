...
import app.catapult.extensions.repeatOnAttached
...
import app.catapult.launcher.util.broadcastReceiverFlow
...
class IcuDateTextView @JvmOverloads constructor(
    ...
    init {
        repeatOnAttached {
            val calendarSelectionEnabled = settings.smartspaceCalendarSelectionEnabled.firstBlocking()
            val calendarFlow = if (calendarSelectionEnabled) {
                    settings.smartspaceCalendar.get()
                } else {
                    flowOf(settings.smartspaceCalendar.defaultValue)
            }
            val optionsFlow = DateTimeOptions.fromSettings()
...
data class DateTimeOptions(
    ...
    companion object {
        fun fromSettings() = combine(
            settings.smartspaceShowDate.get(),
            settings.smartspaceShowTime.get(),
            settings.smartspaceTimeFormat.get()
